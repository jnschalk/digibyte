...
## How to see information about your Tor configuration via DigiByte Core

There are several ways to see your local onion address in DigiByte Core:
...
knows when upgrading to DigiByte Core v22.0 and up that supports Tor v3 only.
...
DigiByte Core makes use of Tor's control socket API to create and destroy
ephemeral onion services programmatically. This means that if Tor is running and
proper authentication has been configured, DigiByte Core automatically creates an
onion service to listen on. The goal is to increase the number of available
onion nodes.

This feature is enabled by default if DigiByte Core is listening (`-listen`) and
it requires a Tor connection to work. It can be explicitly disabled with
`-listenonion=0`. If it is not disabled, it can be configured using the
`-torcontrol` and `-torpassword` settings.

To see verbose Tor information in the digibyted debug log, pass `-debug=tor`.
...
methods to be configured: cookie authentication or digibyted's `-torpassword`
...
## 3. Manually create a DigiByte Core onion service
...
- Do not add anything but DigiByte Core ports to the onion service created in section 3.
