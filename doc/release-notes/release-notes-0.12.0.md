DigiByte Core version 0.12.0 is now available from:

  <https://digibyte.org/bin/digibyte-core-0.12.0/>
...
pre-0.12 versions of DigiByte Core or other software.

If you want to downgrade after you have done a reindex with 0.12.0 or later,
you will need to reindex when you first start DigiByte Core version 0.11 or
earlier.
...
[libsecp256k1](https://github.com/digibyte-core/secp256k1) instead of OpenSSL.
...
(https://github.com/digibyte/bips/blob/master/bip-0130.mediawiki)
...
DigiByte Core 0.12 will have a strict maximum size on the mempool. The
...
DigiByte Core 0.12 also introduces new default policy limits on the length and
...
DigiByte Core 0.12 nodes. DigiByte Core will only allow replacement of
transactions which have any of their inputs' `nSequence` number set to less
than `0xffffffff - 1`.  Moreover, a replacement transaction may only be
accepted when it pays sufficient fee, as described in [BIP 125]
(https://github.com/digibyte/bips/blob/master/bip-0125.mediawiki).
...
Note that the wallet in DigiByte Core 0.12 does not yet have support for
...
In DigiByte Core 0.12, when mempool limit has been reached a higher minimum
...
notes of v0.11.0](https://github.com/digibyte/digibyte/blob/v0.11.0/doc/release-notes.md#block-file-pruning).

`NODE_BLOOM` service bit
------------------------

Support for the `NODE_BLOOM` service bit, as described in [BIP
111](https://github.com/digibyte/bips/blob/master/bip-0111.mediawiki), has been
...
* OP_NOP2 has been renamed to OP_CHECKLOCKTIMEVERIFY by [BIP 65](https://github.com/digibyte/bips/blob/master/bip-0065.mediawiki)
...
- #6214 `87406aa` [OSX] revert renaming of DigiByte-Qt.app and use CFBundleDisplayName (partial revert of #6116) (Jonas Schnelli)
- #6218 `9d67b10` build/gitian misc updates (Cory Fields)
- #6269 `d4565b6` gitian: Use the new digibyte-detached-sigs git repo for OSX signatures (Cory Fields)
...
- #6600 `ef4945f` Include digibyte-tx binary on Debian/Ubuntu (Zak Wilcox)
...
- #6390 `6a73d66` tests: Fix digibyte-tx signing test case (Wladimir J. van der Laan)
...
- #6730 `cb4d6d0` build: Remove dependency of digibyte-cli on secp256k1 (Wladimir J. van der Laan)
