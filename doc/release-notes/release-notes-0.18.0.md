...
`/Applications/DigiByte-Qt` (on Mac) or `bitcoind`/`bitcoin-qt` (on
Linux).
...
recommended to use DigiByte Core on unsupported systems.
...
`~bitcoin/.bitcoin`. This change makes DigiByte Core more consistent with
...
- A new document introduces DigiByte Core's BIP174 [Partially-Signed
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent DigiByte-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  DigiByte Core to libssl vulnerabilities in the past, builders who don't
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when DigiByte Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of DigiByte Core, subsequently importing
...
- A new `bitcoin-wallet` tool is now distributed alongside DigiByte
...
This section describes planned changes to DigiByte Core that may affect
other DigiByte software and services.

- Since version 0.16.0, DigiByte Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with DigiByte Core 0.20 (expected about
  a year after 0.18), DigiByte Core will default to native segwit
  ...
  to bech32 addresses, and if the DigiByte Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in DigiByte Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any DigiByte Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, DigiByte Core
...
- A sub-project of DigiByte Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with DigiByte Core.  See their [project
...
- This release changes the Random Number Generator (RNG) used from
  OpenSSL to DigiByte Core's own implementation, although entropy
  gathered by DigiByte Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves DigiByte Core a
...
- On macOS, DigiByte Core now opts out of application CPU throttling
...
- #11625 Add DigiByteApplication & RPCConsole tests (ryanofsky)
...
- #11526 Visual Studio build configuration for DigiByte Core (sipsorcery)
...
- #12775 Integration of property based testing into DigiByte Core (Christewart)
