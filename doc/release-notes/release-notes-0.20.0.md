...
#### Removal of BIP61 reject network messages from DigiByte Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since DigiByte Core version 0.18.0.
...
- Testing or debugging of implementations of the DigiByte P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of DigiByte Core. DigiByte Core logs debug messages
...
- OpenSSL is no longer used by DigiByte Core.  (#17265)

- BIP70 support has been fully removed from DigiByte Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/digibyte/digibyte/issues/16599),
  [PR #16702](https://github.com/digibyte/digibyte/pull/16702), and the
  `digibyted help` for more information.  This option is experimental and
...
- All custom settings configured when DigiByte Core starts are now
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed DigiByte Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/digibyte-core/HWI) for
  signing.  Future versions of DigiByte Core should support a GUI option
...
- DigiByte Core's automatically-generated source code documentation is
  now available at https://doxygen.digibytecore.org.  (#17596)
...
- The `digibyte-cli` utility used with the `-getinfo` parameter now
...
- The `digibyte-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17741 Included `test_digibyte-qt` in msvc build (sipsorcery)
...
- #18702 Fix ASLR for digibyte-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate digibyte schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest digibyte.conf (sdaftuar)
...
- #18166 ci: Run fuzz testing test cases (digibyte-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone digibyte-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about DigiByte Core files (hebasto)
...
- #17119 Fix broken digibyte-cli examples (andrewtoth)
...
- #17503 Remove bitness from digibyte-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.digibytecore.org (laanwj)
...
- #17648 Rename wallet-tool references to digibyte-wallet (hel-o)
...
- #18341 Replace remaining literal DGB with `CURRENCY_UNIT` (domob1812)
...
- #16115 On digibyted startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from digibyte-cli -getinfo (malevolent)
